...
*/
class Like {
	constructor(private _numberLikes: number, private _isOn: boolean) {}

	onClick() {
		if (this._isOn) {
			this._numberLikes--
		} else this._numberLikes++
...
let likeComponent = new Like(5, false)
likeComponent.onClick()
console.log(`the number of likes is ${likeComponent.numberLikes}`)
